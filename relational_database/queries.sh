...
echo "$($PSQL "SELECT name FROM (SELECT name FROM games LEFT JOIN teams on teams.team_id=games.winner_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT name FROM games LEFT JOIN teams on teams.team_id=games.opponent_id WHERE year=2014 AND round='Eighth-Final') q ORDER BY name")"
